  ...
  return (
    <section id="practice" aria-labelledby="practice-heading" className="scroll-mt-[120px] pb-12 min-[640px]:pb-14">
      <div className="mb-4 flex items-center justify-between gap-3">
        ...
        <Link to="/problem" className="hidden items-center gap-2 text-[12px] text-muted [&:hover]:text-accent-text min-[640px]:inline-flex min-[640px]:min-h-8">View all problems<Icon name="arrow" width="15" height="15" /></Link>
      </div>
      <div className="grid gap-3.5 min-[640px]:grid-cols-2">
        <Link to="/problem" className="group block overflow-hidden rounded-[15px] border border-line bg-surface shadow-[inset_0_1px_0_var(--color-highlight)] transition-[border-color,transform] duration-180 ease-[ease] [&:hover]:border-accent-text [&:hover]:[transform:translateY(-2px)]">
