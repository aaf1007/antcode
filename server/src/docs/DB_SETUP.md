...
- **Node.js 22.12+** (Node 24 LTS recommended) and `npm install` already run once in the repo.
...
This reads `server/src/db/prisma/contract.prisma` and writes two generated files next to it:
...
The seed script reads `server/src/db/raw/leetcode_problems.json` and populates the
problem catalog and related tables. It clears those tables before inserting,
so run it for initial setup or when you intend to reload the catalog.
...
Always start by editing `server/src/db/prisma/contract.prisma`, then re-emit:
...
`migration plan` writes a reviewable, content-hashed migration package under `server/src/db/migrations/app/<timestamp>_<name>/`, and `db migrate` applies it. Commit the generated directory along with your contract change — that is how everyone else gets the same schema.
